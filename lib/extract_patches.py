...
from help_functions import group_images_new
...
def get_data_training_color_label(DRIVE_train_imgs_original,
    ...
    train_labels = load_hdf5(DRIVE_train_groudTruth) #masks always the same
    ...
    return patches_imgs_train, train_labels
...
def get_data_testing_overlap_color(DRIVE_test_imgs_original, DRIVE_test_groudTruth, Imgs_to_test, patch_height, patch_width, stride_height, stride_width):
    ### test
    test_imgs_original = load_hdf5(DRIVE_test_imgs_original)
...
def extract_random(full_imgs, patch_h, patch_w, N_patches):
...
#return only the pixels contained in the FOV, for both images and masks
def pred_whole_img(data_imgs,data_masks):
